...
(def transport-test-suite
  (test-suite "lsp/transport"
    ...
    ;; --- read-headers ---
    ;; read-headers uses read-line (character I/O), so open-input-string works
    (test-case "read-headers: valid Content-Length"
      (let ((port (open-input-string "Content-Length: 42\r\n\r\n")))
    ...
    (test-case "read-headers: missing Content-Length"
      (let ((port (open-input-string "Content-Type: text/plain\r\n\r\n")))
        (check (read-headers port) => #f)))
    ...
    ;; --- round trip via file ---
    ;; write-message writes bytes; read-message uses read-line then read-subu8vector.
    ;; Must use buffering: #f to allow mixing character and byte I/O on same port.
    (test-case "write-message then read-message: round trip via file"
      (let* ((body (json-object->string (hash ("jsonrpc" "2.0") ("method" "test"))))
        ...
        (let ((out (open-output-file tmp-path)))
          ...
          (close-port out))
        ;; Read back with buffering: #f to allow mixed char/byte reads
        (let ((in (open-input-file (list path: tmp-path buffering: #f))))
